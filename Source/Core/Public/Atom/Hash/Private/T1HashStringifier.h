...
namespace Atom::Private
{
    ...
    {
        pub cexpr fn ToStr(const T1Hash& hash) -> Str
        {
            Str str;
            WriteStr(hash, str);
            return str;
        ...
        pub cexpr fn WriteStr(const T1Hash& hash, ROutput<Char> auto&& out)
        {
...
namespace Atom
{
    ...
    {
        pub cexpr fn Convert(const T1Hash& hash) -> Str
        {
            return Private::T1HashStringifier<T1Hash>()
                .ToStr(hash);
        }

        pub cexpr fn Convert(const T1Hash& hash, ROutput<Char> auto&& out)
        {
            return Private::T1HashStringifier<T1Hash>()
                .WriteStr(hash, out);
        }
