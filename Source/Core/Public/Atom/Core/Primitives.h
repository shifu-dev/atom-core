...
namespace Atom
{
    ...
    {
    public:
        static consteval auto CountDigits(auto n) -> _usize
        {
            _usize count = 0;
            while (n > 0)
            {
        ...
        static consteval auto MinDigitsCount() -> _usize
        {
            return CountDigits(TLimits::min());
        }

        static consteval auto MaxDigitsCount() -> _usize
        {
            return CountDigits(TLimits::max());
        }

        template <typename N>
        static constexpr auto ToString(char* str, N val) -> char*
        {
            bool neg = false;
    ...
    {
    private:
        template <typename T_, typename TLimits_>
        friend class Num;

    private:
        using _T = T;
        using _Impl = _NumImpl<T, TLimits>;
        using Self = Num<T, TLimits>;

    public:
        using TStr = NumStr<_Impl::MaxDigitsCount()>;
    ...
    public:
        ...
        ///
        /// ----------------------------------------------------------------------------------------
        constexpr auto countDigits() const
        {
            return _Impl::CountDigits(_val);
        }
...
namespace Atom
{
    ...
    using i8 = Num<_i8, std::numeric_limits<_i8>>;
    using i16 = Num<_i16, std::numeric_limits<_i16>>;
    using i32 = Num<_i32, std::numeric_limits<_i32>>;
    using i64 = Num<_i64, std::numeric_limits<_i64>>;

    using u8 = Num<_u8, std::numeric_limits<_u8>>;
    using u16 = Num<_u16, std::numeric_limits<_u16>>;
    using u32 = Num<_u32, std::numeric_limits<_u32>>;
    using u64 = Num<_u64, std::numeric_limits<_u64>>;
    ...
    using f32 = Num<_f32, std::numeric_limits<_f32>>;
    using f64 = Num<_f64, std::numeric_limits<_f64>>;
    using f128 = Num<_f128, std::numeric_limits<_f128>>;

    using isize = Num<_isize, std::numeric_limits<_isize>>;
    using usize = Num<_usize, std::numeric_limits<_usize>>;
