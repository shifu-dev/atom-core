...
namespace Atom
{
    ...
    concept _RIterOf = requires { requires RConvertibleTo<typename TIter::TElem, T>; };

    template <typename TIter>
    concept _RMutIter = requires(TIter it) {
        {
            it.mutValue()
        } -> RConvertibleTo<typename TIter::TElem&>;
    };
    ...
    concept RIter = requires(TIter it, const TIter cit) {
        ...
        {
            cit.value()
        } -> RConvertibleTo<const typename TIter::TElem&>;
        {
            it.next()
        } -> RConvertibleTo<TIter&>;
    };
    ...
    concept RArrayIter = requires(const TIter cit) {
        ...
        {
            cit.data()
        } -> RConvertibleTo<MemPtr<typename TIter::TElem>>;
    };
    ...
    concept RMutArrayIter = requires(TIter it) {
        ...
        {
            it.mutData()
        } -> RConvertibleTo<MutMemPtr<typename TIter::TElem>>;
    };
